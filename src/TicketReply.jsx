...
import { IoCaretDownSharp, IoCloseOutline, IoImageOutline, IoLinkOutline, IoSendSharp } from "react-icons/io5";
...
const TicketReply = () => {
    const { id } = useParams();
    const { user } = useAuth();
    const { formatDate, updateTicket, ticketsRefetch, setShowDeleteModal, setSelectedTicket, showNoteModal, setShowNoteModal, refetchStats } = useTickets();
    ...
    const statusOptions = [
        ...
        { value: 'closed', label: 'Closed', color: 'bg-gray-200 text-gray-900 border-gray-200' },
        { value: 'reopened', label: 'Reopen', color: 'bg-blue-100 text-blue-800 border-blue-200' }
    ];

    // FETCH TICKET DATA
    const { data: singleTicket = [], refetch: singleTicketRefetch, isFetching: isFetchingSingleTicket, isLoading: isLoadingSingleTicket } = useQuery({
        queryKey: ['singleTicket', id],
        queryFn: async () => {
            const res = await axiosSecure.get(`/api/v1/ticket/get-ticket-employee/${id}`);
            return res.data;
        }
    });
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                const filteredUsers = allUsers.filter(searchUser =>
                    !mentionedUsers.some(mentioned => mentioned.username === searchUser.username) &&
    ...
    const handleMentionSelect = (selectedUser) => {
        ...
        setTimeout(() => {
            if (richInputRef.current) {
                ...
                richInputRef.current.setSelectionRange(newCursorPosition, newCursorPosition);
            }
        }, 0);
    };
        ...
        try {
            ...
            const response = await fetch(
                'https://files.remoteintegrity.com/api/files/faba4ad7-862a-4a20-9f33-73cf0286aa4c/upload',
                {
                    method: 'POST',
                    headers: {
                        'x-api-key': '99cd5d53a736d4f65458e4a943e86dcb6701ddf10fc7d09efca918933975bdf0',
                    },
                    body: formData,
                }
            );
        ...
        try {
            ...
            if (selectedFiles.length > 0) {
                const uploadPromises = selectedFiles.map(async (selectedFile) => {
                    try {
                        const uploadedFile = await uploadFile(selectedFile.file, selectedFile.id);
            ...
            const response = await axiosSecure.post("/api/v1/ticket/send-message", payload);
            ...
            if (response.data && response.data.data) {
                ...
                try {
                    const messagesResponse = await axiosSecure.get(`/api/v1/ticket/get-messages/${id}`);
    ...
    const getFileTypeIcon = (file) => {
        if (file.file.type.startsWith('image/')) {
            ...
            return '📄';
        } else if (file.file.type.includes('word') || file.file.type.includes('document')) {
            return '📝';
        } else if (file.file.type.includes('sheet') || file.file.type.includes('excel')) {
            return '📊';
        } else if (file.file.type.includes('zip') || file.file.type.includes('rar')) {
            return '📦';
        } else {
            return '📎';
        }
                ...
                <div className="flex-1 space-y-3 flex flex-col">
                            ...
                            {ticket.attachments && ticket.attachments.length > 0 && (
                                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                                    {ticket.attachments.map((attachment, index) => {
                                        const isImage = isImageFile(attachment.filename, attachment.type);
                    ...
                    <div className="flex-1 flex flex-col pt-4">
                        <div className="flex items-center gap-3">
                            <h3 className="text-lg font-semibold mb-5 text-gray-800">Responses</h3>
                                ...
                                <div className="border border-gray-300 rounded-[4px] p-3 bg-gray-50">
                                    ...
                                    <div className="space-y-2 max-h-40 overflow-y-auto">
                                        {selectedFiles.map((fileObj) => (
                                            <div key={fileObj.id} className="flex items-center justify-between p-2 bg-white rounded border">
                                                <div className="flex items-center space-x-3 flex-1 min-w-0">
                                                    {fileObj.preview ? (
                                                        ...
                                                        <div className="w-10 h-10 bg-gray-200 rounded border flex items-center justify-center flex-shrink-0">
                                                            <span className="text-lg">{getFileTypeIcon(fileObj)}</span>
                                                        </div>
                                                    )}
                                    ...
                                    <form onSubmit={handleSendComment} className="p-3">
                                                                ...
                                                                >
                                                                    <div className="w-8 h-8 rounded-full bg-gray-300 flex items-center justify-center overflow-hidden flex-shrink-0">
                                                                        {suggestion.avatar ? (
                                                                            ...
                                                                            <span className="text-sm font-medium text-gray-600">
                                                                                {suggestion.name?.charAt(0)?.toUpperCase() || 'U'}
                                                                            </span>
                                                                        )}
                                                                    </div>
                                                                    <div className="flex-1 min-w-0">
                                                                        ...
                                                                        <div className="text-xs text-gray-500">
                                                                            @{suggestion.username} • {suggestion.position || 'No position'}
                                                                        </div>
                                                                    </div>
                                                                </button>
                                                            ))}
                                        ...
                                        <div className="flex items-center justify-between pt-2 border-t border-gray-200">
                                            ...
                                            <button
                                                ...
                                                disabled={isSending || !hasTicketAccess || (!newComment.trim() && selectedFiles.length === 0)}
                                                    ...
                                                    <div className="w-10 h-10 rounded-full bg-gray-400 flex items-center justify-center overflow-hidden">
                                                        {comment.sender?.avatar ? (
                                                            ...
                                                            <span className="text-sm font-medium text-white">
                                                                {comment.sender?.name?.charAt(0)?.toUpperCase() || 'U'}
                                                            </span>
                                                        )}
                                                    </div>
                                                </div>
                                                    ...
                                                    <div className="flex items-center gap-2 mb-2">
                                                        ...
                                                        <span className="bg-blue-500 rounded-[6px] px-2 py-0.5 text-white text-xs">
                                                            {comment?.sender?.roleId?.roleName
                                                                ? comment.sender.roleId.roleName.charAt(0).toUpperCase() +
                                                                comment.sender.roleId.roleName.slice(1).toLowerCase()
                                                                : "Employee"}
                                                        </span>
                                                        ...
                                                        {comment.content && (
                                                            <div>
                                                                <p className="text-sm text-gray-800 leading-relaxed whitespace-pre-wrap break-words">
                                                                    {renderCommentContent(comment.content, comment.participants)}
                                                                </p>
                                                            </div>
                                                        )}
                                                                        ...
                                                                        <div key={index} className="group">
                                                                            {isImage ? (
                                                                                ...
                                                                                <div className="relative">
                                                                                    ...
                                                                                    {index === 2 && comment.attachments.length > 3 && (
                                                                                ...
                                                                                <div
                                                                                    ...
                                                                                    onClick={() => window.open(attachment.url, '_blank')}
                                                                                >
                                                                                    <span className="text-lg mb-1">{getFileIcon(attachment.filename, attachment.type)}</span>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-2 border-b pb-3">
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                >
                                    <span
                                        className={`inline-flex items-center gap-1 px-2 py-1 rounded-[4px] text-sm font-medium transition-opacity border ${getStatusColor(ticket.status)}`}
                                    >
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <div
                                    className="relative"
                                    ref={el => dropdownRefs.current[`${ticket._id}-priority`] = el}
                                >
                                    <span
                                        className={`inline-flex items-center gap-1 px-2 py-1 rounded-[4px] text-sm font-medium transition-opacity border ${getPriorityColor(ticket.priority)}`}
                                    >
                                        {priorityOptions.find(opt => opt.value === ticket.priority)?.label || ticket.priority}
                                    </span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-3 border-b pb-2">
                            ...
                            {ticket?.division?.roleName ? (
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <ParticipantsAvatars
                                        participants={ticket?.participants.filter(id => id !== user?._id)}
                                    />
                                </div>
                            }
                        </div>
